...
cd backend-kotlin && JAVA_HOME="$JAVA_21_HOME" ./mvnw clean package -DskipTests
cd ..

# Pre-compile Java backend
echo "Pre-compiling Java backend (baseline)..."
cd backend-java && JAVA_HOME="$JAVA_21_HOME" ./mvnw clean package -DskipTests
cd ..

# Run Benchmarks
run_benchmark "Kotlin-SpringBoot" "backend-kotlin" "JAVA_HOME="$JAVA_21_HOME" java -Xms200m -Xmx1g -jar target/spring-inventory-0.0.1-SNAPSHOT.jar"
run_benchmark "Java-SpringBoot-21" "backend-java" "JAVA_HOME="$JAVA_21_HOME" java -Xms200m -Xmx1g -jar target/spring-inventory-0.0.1-SNAPSHOT.jar"
run_benchmark "Java-SpringBoot-25" "backend-java" "JAVA_HOME="$JAVA_25_HOME" java -Xms200m -Xmx1g -jar target/spring-inventory-0.0.1-SNAPSHOT.jar"
